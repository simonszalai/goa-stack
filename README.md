...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create goa_stack-e7c0
  fly apps create goa_stack-e7c0-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app goa_stack-e7c0
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app goa_stack-e7c0-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'goa_stack-e7c0-staging' does not match app name in config file 'goa_stack-e7c0'
  > ```
  >
